...
const RejectRemarksForm = () => {
  ...
  if (encryptedData) {
    ...
    refString = decrypted.rows || []; // array of vouchers
  }
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(uncheckedRows, "uncheckedRows");
    try {
      if (uncheckedRows.length > 0) {
        const encryptedData = encrypt({ refs: uncheckedRows.join(",") });
        await axios.get(
          `${process.env.REACT_APP_API}/api/approve?q=${encodeURIComponent(
            encryptedData
          )}`
        );
      }
      ...
      const result = await axios.post(
        `${process.env.REACT_APP_API}/api/reject`,
        {
          data: {
...
export default RejectRemarksForm;
